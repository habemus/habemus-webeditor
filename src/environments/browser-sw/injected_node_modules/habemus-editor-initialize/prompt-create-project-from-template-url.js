/**
 * Checks whether the requested URL contains a query string prop
 * named "template_url". If so, prompts the user to create a new
 * project based on the template files.
 */
...
function parseTemplateURL() {
  ...
  var templateURL = parsedURL.query.template_url;

  if (!templateURL) {
    return false;
  }

  if (!HTTP_RE.test(templateURL)) {

    var manipulated = Object.assign({}, parsedURL, {
      pathname: templateURL,
    });

    delete manipulated.search;
    delete manipulated.query.template_url;

    templateURL = url.format(manipulated);
  }
